...
export default createRule<[options: IOptions], 'noMixCoreModuleFileComputed' | 'noMixRequire'>({
  ...
  create(context, appliedOptions) {
     ...
     */
    const getDeclarationType = (initExpression: TSESTree.Expression | null): string => {
      if (!initExpression) {
        // "var x;"
     ...
     */
    const inferModuleType = (initExpression: TSESTree.Expression | null): string => {
      if (initExpression?.type === 'MemberExpression') {
        // "var x = require('glob').Glob;"
        return inferModuleType(initExpression.object);
      }

      if (!(initExpression as TSESTree.CallExpression | null)?.arguments.length) {
        // "var x = require();"
        return REQ_COMPUTED;
      }

      const [arg] = (initExpression as TSESTree.CallExpression).arguments;
     ...
     */
    const isMixed = (declarations: TSESTree.VariableDeclarator[]): boolean => {
     ...
     */
    const isGrouped = (declarations: TSESTree.VariableDeclarator[]): boolean => {
